         ...
         <CstmrCdtTrfInitn>
            <GrpHdr>
               ...
               <InitgPty>
				  ...
				  <!-- Removed from Cheques as WorldLink requires no value
                  <Id>
                     <OrgId>
                        <xsl:choose>
                  ...
                  <DbtrAgt>
                     <FinInstnId>
                        <xsl:if test="not(BankAccount/SwiftCode='')">
							<!-- Hardcoding for San Francisco acc 30736705 -->
							<xsl:if test="not(BankAccount/BankAccountNumber='30736705')">
								<BIC>
									<xsl:value-of select="BankAccount/SwiftCode" />
								</BIC>
							</xsl:if> <!-- End Hardcoding for San Francisco acc 30736705 -->
						</xsl:if>
					 ...
					 <!-- Hardcoding for San Francisco acc 30736705 -->
						<xsl:if test="(BankAccount/BankAccountNumber='30736705')">
							<BrnchId><Id>930</Id></BrnchId>
						</xsl:if> <!-- End Hardcoding for San Francisco acc 30736705 -->
                     <!--<xsl:if test="(BankAccount/SwiftCode='')">
								<xsl:if test="not(BankAccount/BranchNumber='')">
									<BrnchId>
                           ...
                           <xsl:for-each select="DocumentPayable">
                              <Strd>
                                 <RfrdDocInf>
                                    ...
                                    <Nb>
                                       <xsl:value-of select="substring(translate(DocumentNumber/ReferenceNumber,'&amp;;#*=$![~`]{\}%@' ,''),1,35)" />
                                    </Nb>
                                    <RltdDt>
                                 ...
                                 <RfrdDocAmt>
                                    <xsl:if test="not(TotalDocumentAmount/Value = 0)">
                                       <DuePyblAmt>
                                          ...
                                          <xsl:value-of select="translate(format-number(TotalDocumentAmount/Value, '##0.00'),'-','')" />
                                       </DuePyblAmt>
                                    </xsl:if>
                                    ...
                                    <xsl:if test="not(TotalDocumentTaxAmount/Value = 0)">
                                       <xsl:variable name="documentTax" select="TotalDocumentTaxAmount/Value" />
                                       <xsl:if test="$documentTax &gt; 0">
                                          <xsl:value-of select="format-number($documentTax, '##0.00')" />
                                          <TaxAmt>
                                             <xsl:attribute name="Ccy">
                                 ...
                                 <CdtrRefInf>
                                    <Tp>
                                       ...
                                       <Issr>
                                          <xsl:value-of select="substring(translate($IssurName,'&amp;;#*=$![~`]{\}%@' ,''),1,35)" />
                                       </Issr>
                                    </Tp>
                                    <Ref>
                                       <xsl:choose>
