/* APPSQRY.VTRIALBALANCE View used by Essbase to import data. 2008 onwards
--  Now Excludes ABS Entities */
CREATE OR REPLACE FORCE VIEW "APPSQRY"."VTRIALBALANCE" ("PERIOD_NAME", "SEGMENT1", "SEGMENT2", "SEGMENT3", "SEGMENT4", "SEGMENT5",
       "SEGMENT6", "SEGMENT7", "OPEN BAL", "DEBIT", "CREDIT", "NET MOVEMENT", "CLOSE BAL", "CURRENCY_CODE", "ACCOUNT_TYPE",
       "ASSET_CODE")
AS
 SELECT GB.PERIOD_NAME
     ...
     , (NVL (GB.PERIOD_NET_DR, 0) + NVL (GB.BEGIN_BALANCE_DR, 0)) - (NVL ( GB.PERIOD_NET_CR, 0) + NVL (GB.BEGIN_BALANCE_CR, 0))
       "CLOSE BAL"
     , GB.CURRENCY_CODE
