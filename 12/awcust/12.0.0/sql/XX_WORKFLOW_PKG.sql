...
PROCEDURE credit_card(itemtype IN VARCHAR2
                                ,itemkey  IN VARCHAR2
                                ,actid    IN NUMBER
                                ,FUNCMODE in varchar2
                                ,RESULT   IN OUT VARCHAR2);
...
BEGIN
     ...
     dbms_output.put_line('2');
          ...
          WHERE (REPLACE(REPLACE(full_NAME,'''',''),',','') = V_LINE_MANAGER or last_name = V_LINE_MANAGER);
...
PROCEDURE credit_card(itemtype IN VARCHAR2
  ...
  BEGIN
    ...
    and a.ASSIGNMENT_ID = TO_NUMBER(ITEMKEY);
...
-- update Assignments
          update per_all_assignments_f
            set      set_of_books_id = 8
                   ...
                   ,SUPERVISOR_ID = (SELECT PERSON_ID FROM PER_ALL_PEOPLE_F WHERE (REPLACE(GLOBAL_NAME,'''','') = REPLACE(V_MANAGER,'''','') OR LAST_NAME||' '||FIRST_NAME = REPLACE(V_MANAGER,'''',''))) 
                   ,ass_attribute1 = (select person_id from per_all_people_f where (REPLACE(global_name,'''','') = REPLACE(v_manager,'''','') or last_name||' '||first_name = REPLACE(v_manager,'''',''))) 
...
-- Person Specific Values
  ...
  X_JUSTIFICATION         := 'New Emp-Role requested based on user:'||P_COPY_FROM_USER;
...
-- Loop Through Variables
  ...
  dbms_output.put_line('1'||X_REG_SERVICE_TYPE);
  ...
  dbms_output.put_line('8'||X_REQUESTED_END_DATE);
...
BEGIN
     IF ( funcmode = 'RUN' ) THEN
         ...
         dbms_output.put_line('003');
          ...
          from per_all_people_f p,
          FND_USER U
          WHERE (P.GLOBAL_NAME = REPLACE(V_LINE_MANAGER,',','') OR REPLACE(P.LAST_NAME||' '||P.FIRST_NAME,',','') = REPLACE(V_LINE_MANAGER,',','') OR P.LAST_NAME = V_LINE_MANAGER
          or p.FIRST_NAME||' '||p.last_name = REPLACE(V_LINE_MANAGER,',',''))                  
