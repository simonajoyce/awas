CREATE OR REPLACE PACKAGE XX_ULSTER_STMT_INTERFACE_PKG
IS
...
PROCEDURE XX_ULSTER_bank_stmt_load
     (
...
PROCEDURE validate_bank_stmt
     ...
     /*-- returns 0 if loaded successfully, >0 otherwise along with error message.*/
PROCEDURE int_XX_ulster_bank_stmt_std
     (
          x_error_message OUT VARCHAR2,
          x_error_code OUT NUMBER ) ;
...
END XX_ULSTER_STMT_INTERFACE_PKG;
/


CREATE OR REPLACE PACKAGE BODY XX_ulster_STMT_INTERFACE_PKG
IS
     /*--+==========================================================================+*/
     /*--| Name:         XX_ulster_STMT_INTERFACE_PKGBODY        |*/
     /*--| Description:  XX_ulster_STMT_INTERFACE_PKGbody contains the  definition |*/
...
BEGIN
     ...
     END;
     Write_out ( l_org_name||'            ulster - Bank Statement Upload Execution Report             '||TO_CHAR ( SYSDATE, 'DD-MON-YYYY HH:MI:SS' ) ) ;
     Write_out ( '-----------------------------------------------------------------------------------------------' ) ;
     write_out ( '' ) ;

     /*-- Load the statements into cash management*/

     l_file_ext := '\XX_ulster';
     write_debug ( 'bank_statement_load -> calling XX_ulster_bank_stmt_load' ) ;
     XX_ulster_bank_stmt_load ( l_error_message, l_error_code, p_input_file, fileloc ) ;
     write_debug ( 'bank_statement_load ->returned from XX_ulster_bank_stmt_load' ) ;
...
/*--=============================================================================*/
/*--=============================================================================*/
/*--Procedure which will upload XX_ulster Bank format file into Oracle Cash Management*/
PROCEDURE XX_ulster_bank_stmt_load
     (
          x_error_message OUT VARCHAR2,
          x_error_code OUT NUMBER,
...
BEGIN
     write_debug ( 'XX_ulster_bank_stmt_load -> Begining of procedure' ) ;
     WRITE_LOG ( 'XX_ulster_bank_stmt_load -> Begining of procedure' ) ;
     submit_conc_request ( p_application =>'CE', p_program =>'CESLRPROULSTER', p_sub_request => FALSE, p_parameter1 => p_dir_path||'/'||p_input_file, p_parameter2 => NULL, p_parameter3 => NULL, p_parameter4 => NULL, p_parameter5 => NULL, p_parameter6 => NULL, p_parameter7 => NULL, p_parameter8 => NULL, p_parameter9 => NULL, p_parameter10 => NULL, p_parameter11 => NULL, p_parameter12 => NULL, p_parameter13 => NULL, p_parameter14 => NULL, p_parameter15 => NULL, p_parameter16 => NULL,x_request_id =>l_request_id ) ;
     /*-- wait till the sqlloader program is completed*/
     IF l_request_id <> 0 THEN
          wait_conc_request ( l_request_id, 'Wait for bank statement loader program to complete', l_phase, l_code ) ;
     ELSE
          x_error_message := 'Failed to submit request to load XX_ulster Bank Statement' ;
          raise e_return_error;
     END IF;
     write_log ( 'XX_ulster_bank_stmt_load -> after waiting for the request' ) ;
     IF ( ( l_phase = 'Completed' ) AND ( l_code IN ( 'Normal', 'Warning' ) ) ) THEN
          ...
          if l_error_code = 0 then
               int_xx_ulster_bank_stmt_std ( l_error_message, l_error_code ) ;
               if l_error_code <> 0 then
                    write_debug ( 'Failed transferring data to standard tables' ) ;
                    x_error_message := 'Failed trasferring data to standard tables';
                    raise e_return_error;
               ELSE
                    write_debug ( 'XX_ulster Bank Statement loaded into the standard interface tables successfully.' ) ;
               END IF;
          ELSE
...
IS
     ...
     IS         select  bank_account_num,
                         attribute1
                 FROM
                    xx_ulster_stmt_lines_int x
                 WHERE NOT EXISTS
                    (SELECT
                              1
                            FROM
                           ...
                           WHERE
                              ...
                              and aba.account_owner_org_id = 85
                              and aba.bank_account_num = x.bank_account_num
                    ) ;
  /*
     ...
     IS
          SELECT DISTINCT
                    ...
                    BANK_ACCOUNT_NUM
                  FROM
                    XX_ulster_STMT_LINES_INTERFACE ISL
                 WHERE
...
IS
     ...
     CURSOR XX_ulster_INTERFACE_STD
     IS
          select  isl.trx_date                   statement_number,
                    isl.trx_date trx_date                 ,
                    isl.bank_account_num stmt_ac              ,
                    sum ( case when isl.amount > 0 then 1 else 0 end) credit_trx,
                    sum ( case when isl.amount < 0 then 1 else 0 end) debit_trx  ,
                    sum ( case when isl.amount > 0 then isl.amount else 0 end) credit_amount,
                    sum ( case when isl.amount < 0 then isl.amount else 0 end) debit_amount   ,
                    ABB.BANK_NAME                                                                        ,
                 ...
                 GROUP BY isl.trx_date,
                    ...
                    aba.currency_code                                                                    ,
                    aba.bank_account_id
                 order by isl.bank_account_num,isl.trx_date;




     L_XX_ulster_INTERFACE_STD XX_ulster_INTERFACE_STD%ROWTYPE;
...
BEGIN
     write_debug ( 'int_xx_ulster_bank_stmt_std-> Begining of the procedure' ) ;

     OPEN XX_ulster_INTERFACE_STD;
     LOOP
          FETCH XX_ulster_INTERFACE_STD INTO L_XX_ulster_INTERFACE_STD;
          EXIT
     WHEN XX_ulster_INTERFACE_STD%NOTFOUND;
          /*-- GENERATE STATEMENT NUMBER*/
           ...
           INSERT
                  INTO
                    ...
                    VALUES
                    (
                         l_xx_ulster_interface_std.statement_number,
                         L_XX_ulster_INTERFACE_STD.STMT_AC,
                         L_XX_ulster_INTERFACE_STD.BANK_NAME       ,
                         L_XX_ulster_INTERFACE_STD.BANK_BRANCH_NAME,
                         L_XX_ulster_INTERFACE_STD.TRX_DATE        ,
                         L_XX_ulster_INTERFACE_STD.CURRENCY_CODE   ,
                         G_ORG_ID                              ,
                         L_XX_ulster_INTERFACE_STD.DEBIT_AMOUNT    ,
                         L_XX_ulster_INTERFACE_STD.CREDIT_AMOUNT   ,
                         L_XX_ulster_INTERFACE_STD.DEBIT_TRX       ,
                         L_XX_ulster_INTERFACE_STD.CREDIT_TRX      ,
                         'N'                                   ,
                         G_USER_ID                             ,
                         SYSDATE                               ,
                         G_USER_ID                             ,
           ...
           INSERT
             ...
             SELECT L_XX_ulster_INTERFACE_STD.STMT_AC,
                    l_xx_ulster_interface_std.statement_number,
                    rownum                               ,
                    l_xx_ulster_interface_std.currency_code   ,
                    case when l.amount > 0 then l.amount else l.amount*-1 end,
                    l.bank_trx_number                     ,
                    l.trx_date       ,
                    case when l.amount > 0 then 'RECEIPT' else 'PAYMENT' end ,
                    l.trx_text                            ,
                    G_USER_ID                             ,
                    SYSDATE                               ,
                    G_USER_ID                             ,
                    sysdate                               ,
                    l.attribute4||l.attribute5
                  from   XX_ULSTER_STMT_LINES_INT l
                 where  l.bank_account_num      = l_xx_ulster_interface_std.stmt_ac
                 and l.trx_date = L_XX_ulster_INTERFACE_STD.TRX_DATE ;


          WRITE_LOG ( 'No of statement lines for the Bank account: '||L_XX_ulster_INTERFACE_STD.STMT_AC||' '||sql%rowcount ) ;
     END LOOP;
     CLOSE XX_ulster_INTERFACE_STD;
     write_debug ( 'int_xx_ulster_bank_stmt_std->  End of procedure' ) ;
...
WHEN OTHERS THEN
     ...
     --CLOSE XX_ulster_INTERFACE_STD;
END int_xx_ulster_bank_stmt_std;
